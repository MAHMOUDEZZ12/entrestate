...
export function AssistantChat() {
  ...
  return (
    <div className="h-full flex flex-col">
       <form onSubmit={handleSendMessage} className="relative p-2 border-b">
            <Input 
                placeholder="Send a message..." 
                className="w-full h-11 pl-4 pr-10 border-0 shadow-none focus-visible:ring-0"
                value={input}
                onChange={(e) => setInput(e.target.value)}
                disabled={isLoading}
                autoComplete="off"
            />
            <Button type="submit" size="icon" disabled={isLoading || !input.trim()} className="absolute right-3 top-1/2 -translate-y-1/2 h-8 w-8">
